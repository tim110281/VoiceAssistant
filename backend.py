...
flags.DEFINE_string("vocab_file", "chinese_roberta_wwm_ext_L-12_H-768_A-12/vocab.txt",
...
## Other parameters
flags.DEFINE_string("train_file", "DRCD/DRCD_training.json",
										"SQuAD json for training. E.g., train-v1.1.json")

flags.DEFINE_string(
		"predict_file", "DRCD/test.json",
...
def ipfnb():
	...
	def input_fn(params):
		...
		return (tf.data.Dataset.from_generator(gen,
			output_types={'input_ids': tf.int32, 'input_mask': tf.int32, 'segment_ids': tf.int32, 'unique_ids': tf.int32},
			output_shapes={'unique_ids': (), 'input_ids': (None, max_seq_len), 'input_mask': (None, max_seq_len),'segment_ids': (None, max_seq_len)})
				.prefetch(1))
...
def parse_predictions(all_examples, all_features, all_results, n_best_size,max_answer_length=30, do_lower_case=True):
	...
	for (example_index, example) in enumerate(all_examples):
		...
		nbest_json = []
		predictions.append((nbest[0].text,probs[0]))
	return predictions
